...
### 편의 / 디테일
- **첨부**(클립보드 Ctrl+V / 파일 선택) — 이미지(실제 썸네일) + 마크다운/텍스트/코드 **문서**(내용을 프롬프트에 인라인, 전 엔진) · 모델 피커 · `@` 멘션 · `/` 슬래시 액션 · Win+H 받아쓰기
- **빠른-응답 버튼** — 어시스턴트가 `A)/B)`·`1./2.` 선택지로 끝내면 컴포저 위에 원클릭 버튼(엔진 무관) · **Enter 전송 / Shift+Enter 줄바꿈**
