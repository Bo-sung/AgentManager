...
1. worktree 공유(메인과 같은 코드) / 독립(자체 worktree) 기본값 — 위임 팝업에서 선택 제공 예정, 기본값 미정.
2. 동시성 cap에서 워커 슬롯 분리 여부(위임이 cap에 막히는 문제).
3. 보고 추출 범위 — 워커 최종 응답 1개(기본) vs 워커 턴 전체 요약(옵션).
4. "합쳐 붙여넣기" 병합 순서 — 위임 순서(기본) vs 완료 순서.
