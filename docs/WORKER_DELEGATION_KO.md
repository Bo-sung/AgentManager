...
- **워커 정의(WorkerDef)** — 생성 시 고정되는 워커별 설정:
  - 엔진·모델·이름
  - **번역 정책**(ON/OFF + 번역 전/후 언어) — 생성 시 고정(§8)
  - **행동 규칙 preamble**(BehaviorPreamble) — 위임 프롬프트 앞에 자동 부착되는 표준 지시문(§7.1). 전역 기본 템플릿 + 워커별 편집.
- `AppSettingsDto` / `state.json`: 워커 풀 정의(WorkerDef 목록) + 세션 Role 영속 + 전역 기본 preamble 템플릿.
...
- **워커 생성 폼**: 엔진·모델·이름 + 번역 정책(고정) + **행동 규칙 preamble**(아래) 편집.

### 7.1 워커 행동 규칙 (Behavior Preamble)

보고 품질을 "추출 범위 결정"이 아니라 **워커 행동 지시**로 보장한다. 각 워커에는 **표준 행동 규칙 preamble**이 있고, 위임 시 실제로 워커에 보내는 프롬프트는:
...
- **목적**: 워커가 항상 작업에 집중하고 **끝에 명확한 보고로 마무리**하게 함 → 보고 추출이 "최종 응답 1개"로 안정적으로 동작(§아래 결정 3).
- **스코프**: 전역 **기본 템플릿**(Settings에서 편집) + **워커별 오버라이드**(생성 시 고정, 풀 관리에서 수정). 번역 정책과 같은 "생성 시 고정" 패턴.
- **기본 템플릿 예시**:
  > 너는 워커 에이전트다. 위임된 작업만 정확히 수행하고 간결하게 처리한다. 불명확하면 합리적으로 가정하고 진행한다(되묻지 않는다). 응답 **맨 끝**에 `## 보고`로 한 작업과 결과·산출물을 3~6줄로 요약한다.
...
## 10.5 확정된 결정 (2026-06-23)

- **워커 가시 위치**: 워커도 실제 세션이므로 **일반 세션과 같은 사이드바 공간**에 두되, 전용 **`WORKERS` 그룹**으로 분리하고 워커 배지/역할 강조색으로 구별. 각 워커 행에 *담당 메인* 라벨 표기. (= 같은 공간 + 명확한 구별)
- **워커 풀 스코프**: **프로젝트 기준**. 워커는 코드/worktree 컨텍스트에서 일하므로 프로젝트별 풀이 자연스럽고, 기존 프로젝트별 세션 그룹화와 일관. 풀 관리(생성/삭제/모델·엔진 변경)는 활성 프로젝트의 `WORKERS` 그룹에서.
- **워커 번역 정책 = 생성 시 고정**: 워커 생성 시 정한 **번역 ON/OFF + 언어(전/후)** 를 그 워커에 **고정**. 위임/보고는 그 워커의 정책대로 처리(생성 시 OFF면 무번역, ON이면 생성 시 언어쌍으로). 위임마다 바뀌지 않음.
- **워커 busy(유휴 워커 없음) 처리**: 위임 시점에 유휴 워커가 없으면 **"유휴 워커 없음" 팝업** 표시. 하단 버튼 **[닫기] / [생성]**. *닫기* = 팝업만 내림(위임 취소). *생성* = **워커 할당 팝업**(새 워커 생성 경로)으로 전환.

## 11. 보고 복귀 & 다수 워커 처리

**단일 보고 복귀**
- 위임은 메인 트랜스크립트의 **DelegationBlock 카드** 1개로 표현. 워커 완료 → 카드가 **"수신 대기(ready)"** 로 전환 → **`보고 붙여넣기`** 누르면 워커 최종 보고가 메인 **입력(Draft)** 에 주입(자동 전송 옵션). 실행 중인 메인 턴에는 절대 주입하지 않음 — ready 상태로 대기.

**다수 워커 동시 완료**
- 각 위임은 **독립 카드**(독립 수명). 메인은 차단되지 않고, 워커들은 cap 내에서 병렬 실행.
- 메인 컴포저에 **"보고 수신함" 배지** — ready 보고 건수 표시.
- 사용자 액션 두 가지:
  - **개별 붙여넣기**: 카드별로 하나씩 메인에 주입.
  - **합쳐 붙여넣기(준비된 N건)**: ready 보고들을 **위임 순서**로 워커 라벨과 함께 병합해 한 번에 주입(예: `## Worker 1 (<name>)\n<report>\n\n## Worker 2 ...`).
- **자동 복귀 토글**: 켜면 메인의 현재 턴 종료 후 ready 보고를 자동 주입. 다수일 때는 **묶음 합쳐 주입**이 기본(메인 턴 난사 방지).
- 실패한 워커는 ready로 안 넣고 카드에 에러 요약만 — 메인 주입은 사용자 확인 후.

## 12. 확정된 운영 세부 (2026-06-23 b)

1. **worktree**: **독립 기본 + 위임 시 "공유" 선택 제공.** 보고 기반 용도에 안전한 격리가 기본, "내 WIP 직접 수정" 류만 공유 opt-in.
2. **동시성 cap**: **워커 전용 cap 분리**(`MaxConcurrentWorkers` 신설). 워커가 메인 슬롯을 소비하지 않아 병렬 오프로드 보장 + MCP 단계 교착 방지.
3. **보고 추출 = 최종 응답 1개**, 품질은 **워커 행동 규칙 preamble**(§7.1)로 보장(끝에 `## 보고` 강제). 카드에서 "전체 포함" 펼침 주입은 옵션으로 유지.
4. **합쳐 붙여넣기 병합 순서 = 위임 순서.** 결정적이고 메인 사고 흐름과 일치.

> 남은 열린 항목 없음. 다음은 MVP 구현 작업분해(§10)로 진행.
