...
## 1.21.16
워커 보고를 "보고 스킬"로 능동 전송 — 완료 감지와 분리. (기능)
- 그전까지 워커 보고는 **턴 완료 시 마지막 응답을 캡처**하는 방식이라, 턴이 매달리거나(v1.21.13~15) 완료 이벤트가 안 오면 보고가 유실·지연됐다. 이제 워커가 **작업을 끝냈다고 판단하면 직접 보고를 쏘는** 방식을 추가.
- **동작**: 워커에 `AGENTMANAGER_REPORT_SPOOL`(=`.am/report/<workerId>/`) env 주입 + 프리앰블이 "끝나면 `report.md`를 이 경로에 써라"라고 지시. `WatchSessionReportSpool`이 그 파일을 감지 → 워커의 현재 Running 태스크에 보고 첨부 + Done 처리 → **origin(컨트롤타워) 보고 수신함에 즉시 반영**. 기존 `ask-user` 스풀과 동일 패턴(파일 write라 cc/gx/agy/pi/커스텀 공용).
- **디커플링**: 워커가 보고를 쏘는 순간 태스크가 Done → 그 뒤 턴이 매달려도 **보고는 이미 도착**. 최근 v1.21.12~15가 땜질하던 "완료 감지 결합"을 근본 제거.
- **폴백 유지**: 워커가 스킬을 안 쓰면 기존 완료-캡처가 받침(태스크가 이미 Done이면 Running 가드로 자동 스킵) → **회귀 위험 0**.
- (검증) 빌드 green(경고 0/오류 0) · 스모크 OK(신규 `worker: REPORT_SPOOL present`). end-to-end(실 워커 report.md→수신함)는 라이브 확인 권장.
